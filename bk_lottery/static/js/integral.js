$(function() {
    ...
    var marginBottom = parseInt($('.integral-list>.integral-item').css('marginBottom'));
    integrals = {};

    var translate = function(id, srcIndex, destIndex, offset, newCount, game1, game2, game3) {
    ...
    var initialize = function() {
        setTimeout(function() {
            TweenMax.staggerTo($('.integral-list>.integral-item'), 5, {
                autoAlpha: 1,
                y: 0,
                ease: Expo.easeOut,
            }, 0.1);
    ...
    $(window).on('resize', function() {
        ...
        marginBottom = parseInt($('.integral-list>.integral-item').css('marginBottom'));
    });

    setInterval(function() {
        $.get(site_url + 'integral/query/', {}, function(data) {
            if (data.result) {
                for (var i = 0; i < data.integrals.length; i++) {
                    ...
                    translate(id, integrals[id].index, i, integrals[id].offset, count,
                        game1, game2, game3);
